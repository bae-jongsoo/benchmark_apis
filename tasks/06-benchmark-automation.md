...
## 목표
`scripts/run_benchmark.py`를 구현하여 Phase 1, Phase 2 벤치마크를 자동으로 실행한다. 서버 구성별로 컨테이너를 기동/종료하고, k6 테스트를 실행하며, docker stats를 수집하고, raw JSON을 1초 단위 집계 CSV로 후처리한다.

## 선행 태스크
- 01-project-setup (config.yaml, docker-compose.yml)
- 02-django-app (Django 앱)
- 03-fastapi-app (FastAPI 앱)
- 04-data-seeding (시드 스크립트)
- 05-k6-test-scripts (k6 스크립트)

## 구현할 파일 목록

### 1. `scripts/run_benchmark.py` — 메인 자동화 스크립트

**사용법**:
```bash
python scripts/run_benchmark.py phase1          # Phase 1만 실행
python scripts/run_benchmark.py phase2          # Phase 2만 실행
python scripts/run_benchmark.py all             # 전체 실행
```

**실행 흐름 (기획서 Section 6 기반)**:
```
1. results/<phase>/ 전체 삭제 후 재생성 (멱등성 보장)
2. DB 시드 확인 (Phase 2만 — 기존 my-postgres 컨테이너 사용)
3. 시나리오별 루프:
   3a. 서버 구성별 루프:
       - 앱 컨테이너 기동 (환경변수로 워커 수, 포트, CPU_HANDLER 주입)
       - 헬스체크 대기 (서버가 응답할 때까지)
       - 워밍업 요청 (5초)
       - k6 부하 테스트 실행 (40초) → raw JSON 기록
         └─ 동시에 docker stats 모니터 → CSV에 CPU/Memory 기록
       - 테스트 종료
       - 앱 컨테이너 종료
4. 후처리: raw JSON → 1초 단위 집계 CSV 생성, elapsed time 정규화
```
...
Django Ninja와 FastAPI 모두 `GET /health` 엔드포인트가 구현되어 있다 (Task 02, Task 03 참조).

#### 1.6 docker stats 수집 (별도 스레드)
...
def collect_docker_stats(
    ...
    epoch_start: float,
    db_container: str | None = None,  # Phase 2에서 DB 컨테이너 모니터링
...
def postprocess_k6_json(raw_json_path: Path, output_csv_path: Path):
    ...
    with open(raw_json_path) as f:
        for line in f:
            ...
            # k6 JSON time format: "2024-01-01T00:00:00.000000Z"
...
## 시간 동기화 (기획서 Section 5)
- k6 실행 직전에 `epoch_start = time.time()`으로 기록
- docker stats 수집도 동일한 `epoch_start` 기준으로 `elapsed_seconds` 계산
- 후처리 시 k6 raw JSON의 첫 번째 타임스탬프를 기준으로 `elapsed_seconds` 정규화
- 대시보드에서는 공통 `elapsed_seconds` 축으로 k6 메트릭과 docker stats를 동일 시간축에 정렬

## 완료 기준
- `scripts/run_benchmark.py`가 `phase1`, `phase2`, `all` 인자를 받아 실행된다
- 각 시나리오/구성별로:
  - 앱 컨테이너를 기동하고 헬스체크를 수행한다
  - 워밍업 후 k6 테스트를 실행한다
  - docker stats를 1초 간격으로 CSV에 기록한다
  - 테스트 후 앱 컨테이너를 종료한다
- Phase 2에서는 DB 컨테이너(my-postgres)의 CPU/Memory도 수집한다
- raw JSON → 1초 단위 집계 CSV 후처리가 동작한다 (`http_req_failed` 메트릭으로 에러율 계산)
- 결과 디렉토리는 실행 시마다 삭제 후 재생성된다 (멱등성)
- 컨테이너 ID를 `docker compose ps -q`로 동적으로 조회한다 (프로젝트명 하드코딩 없음)
- Phase 2 실행 전에 `my-postgres` 컨테이너 실행 여부와 `foods` 테이블 데이터 존재를 확인한다
